...
Parameter = establishModel(InitialParameter,"isPlotModel", false,"isPlotMesh", true,"gridFineness", 'high'); % this function will create the global matrices in workspace (check Parameter), model and mesh diagram in folders: <modelDiagram>, <meshDiagram>
...
[q, dq, t] = calculateResponse(Parameter, [0,10], 2^14, calculateMethod='ode15s', isUseMassMatrix=true); % function to calculate the system response
...
signalProcessing(q, dq, t, Parameter, [0,10], 2^14, SwitchFigure,"plotDofs", [1 2 5 10 111]) % output figures with time span [0,10]; sampling frequency should be the same as above
