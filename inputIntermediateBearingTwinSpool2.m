%% inputIntermediateBearingTwinSpool2 - Configure intermediate bearings with Hertzian contact
%
...
%
%% Syntax
%  OutputParameter = inputIntermediateBearingTwinSpool2(InputParameter)
%
%% Description
% |inputIntermediateBearingTwinSpool2| adds intermediate bearing configuration to
...
%%
function OutputParameter = inputIntermediateBearingTwinSpool2(InputParameter)
...
IntermediateBearing.isHertzian      = [false; false]; % boolean
IntermediateBearing.isHertzianTop   = [false; false];
...
IntermediateBearing.stiffness           =  [1e8, 1e9; 1e6, 0]; % N/m, in column, n*1
IntermediateBearing.stiffnessVertical   =  [1e8, 1e9; 1e6, 0]; % N/m, in column, n*1
IntermediateBearing.stiffnessHV         =  [5e7, 0; 5e5, 0]; % N/m, in column, n*1
IntermediateBearing.stiffnessVH         =  [5e7, 0; 5e5, 0]; % N/m, in column, n*1
IntermediateBearing.damping             =  [500, 100; 300, 0]; % N/s^2, in column, n*1
IntermediateBearing.dampingVertical     =  [500, 100; 300, 0]; % N/s^2, in column, n*1
IntermediateBearing.dampingHV           =  [200, 10; 100, 0]; % N/m, in column, n*1
IntermediateBearing.dampingVH           =  [200, 10; 100, 0]; % N/m, in column, n*1
IntermediateBearing.mass                =  [0.1; 0]; % kg
...
IntermediateBearing.rollerNum        = [0; 0];
IntermediateBearing.radiusInnerRace = [0; 0]; % m
IntermediateBearing.radiusOuterRace = [0; 0]; % m
IntermediateBearing.innerShaftNo = [1; 1]; % indicates Inner shaft No. 
IntermediateBearing.clearance = [0; 0]; % m
IntermediateBearing.contactStiffness = [0; 0]; % N*m^-3/2
IntermediateBearing.coefficient = [1.5; 1.5]; % =3/2 in a ball bearing; = 10/9 in a roller bearing
